    ...
    return (
      <div className='col-md-8 offset-md-2 Settle'>
        <table className='table'>
          ...
          <tbody>
            {Object.entries(this.props.user.minted).map(
              ([username, minted]) => {
                return (
                  <tr key={username}>
                    <td>
                      <img
                        src={`https://res.cloudinary.com/dhvvhdndp/image/twitter_name/${username}`}
                      />
                      &nbsp;&nbsp;
                      <i>
                        <Link to={`/a/${username}`}>@{username}</Link>
                      </i>
                    </td>
                    <td>{minted.available}</td>
                    <td>
                      <SettleButton minted={minted} />
                    </td>
